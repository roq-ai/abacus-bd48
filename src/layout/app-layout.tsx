...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['school-administrator', 'school-owner', 'teacher', 'staff', 'parent'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SchoolInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/schools` : null),
    () => getSchools({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Schools',
      path: isTenantUser ? `/schools/view/${data?.[0]?.id}` : '/schools',
      entity: 'school',
      service: AccessServiceEnum.PROJECT,
      icon: FiBox,
    },
    { name: 'Parents', path: '/parents', entity: 'parent', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
      name: 'School Owners',
      path: '/school-owners',
      entity: 'school_owner',
    ...
    { name: 'Staff', path: '/staff', entity: 'staff', service: AccessServiceEnum.PROJECT, icon: FiBox },
    { name: 'Teachers', path: '/teachers', entity: 'teacher', service: AccessServiceEnum.PROJECT, icon: FiBox },
