...
import { getSchoolOwners } from 'apiSdk/school-owners';
import { SchoolOwnerInterface } from 'interfaces/school-owner';

import { StaffListPage as ShowCaseList } from 'pages/staff';
...
function DashboardPage() {
  ...
  const parentStat = useSWR<PaginatedInterface<ParentInterface>>(
    () => `/parents`,
    () => getParents({ limit: 1, offset: 0 }),
  );

  const schoolOwnerStat = useSWR<PaginatedInterface<SchoolOwnerInterface>>(
    () => `/school-owners`,
    () => getSchoolOwners({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Parents',
      result: parentStat?.data?.totalCount,
      isLoading: parentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'School Owners',
      result: schoolOwnerStat?.data?.totalCount,
      isLoading: schoolOwnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`staff`} passHref legacyBehavior>
                  <Button
