...
import { getSchoolOwners, deleteSchoolOwnerById } from 'apiSdk/school-owners';
import { SchoolOwnerInterface } from 'interfaces/school-owner';

type ColumnType = ColumnDef<SchoolOwnerInterface, unknown>;

interface SchoolOwnerListPageProps {
...
export function SchoolOwnerListPage(props: SchoolOwnerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getSchoolOwners({
        ...
        order: params.order,
        searchTermKeys: ['first_name.contains', 'last_name.contains', 'phone_number.contains', 'address.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SchoolOwnerInterface>>(
    () => `/school-owners?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteSchoolOwnerById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: SchoolOwnerInterface) => {
    if (hasAccess('school_owner', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/school-owners/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('school', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'school',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/schools/view/${record.school?.id}`}>
              {record.school?.name}
            </Link>
          ),
        }
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/school-owners/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('school_owner', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/school-owners/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('school_owner', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="school_owner" />
        </Flex>

        {hasAccess('school_owner', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/school-owners/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
