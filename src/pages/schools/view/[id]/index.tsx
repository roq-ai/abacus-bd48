...
import { getSchoolById } from 'apiSdk/schools';
import { SchoolInterface } from 'interfaces/school';
import { SchoolOwnerListPage } from 'pages/school-owners';
...
function SchoolViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<SchoolInterface>(
    () => (id ? `/schools/${id}` : null),
    () =>
      getSchoolById(id, {
        relations: ['user'],
            ...
            <FormWrapper wrapperProps={{ gap: 3, p: 4 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('school', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/schools/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              <EntityImage entity="school" entityId={id} />
              <List
                w="100%"
                css={{
                  '> li:not(:last-child)': {
              ...
              >
                ...
                <FormListItem label="Established Year" text={data?.established_year} />

                <FormListItem label="School Type" text={data?.school_type} />
