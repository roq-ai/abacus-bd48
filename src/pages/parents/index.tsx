...
type ColumnType = ColumnDef<ParentInterface, unknown>;
...
export function ParentListPage(props: ParentListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ParentInterface>>(
    () => `/parents?params=${JSON.stringify(params)}`,
    fetcher,
  );
